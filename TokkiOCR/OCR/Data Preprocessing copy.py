...
file = json.load(open(f'./kor_dataset/aihub_data/Source/{labeling_filename}'))
json_files = os.listdir(f'./kor_dataset/aihub_data/Label/')

## Separate dataset - train, validation, test
image_files = os.listdir(f'./kor_dataset/aihub_data/Source/images/')
...
## Write json files
with open(f'Source_train_annotation.json', 'w') as file:
	json.dump(train_annotations, file)
with open(f'Source_validation_annotation.json', 'w') as file:
	json.dump(validation_annotations, file)
with open(f'Source_test_annotation.json', 'w') as file:
	json.dump(test_annotations, file)

## Make gt_xxx.txt files
data_root_path = f'./kor_dataset/aihub_data/Source/images/'
save_root_path = f'./deep-text-recognition-benchmark/Source_data/'

obj_list = ['test', 'train', 'validation']
for obj in obj_list:
	total_annotations = json.load(open(f'./Source_{obj}_annotation.json'))
